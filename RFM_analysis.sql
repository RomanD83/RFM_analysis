...
select
	card,
	extract(day from const.max_date::timestamp - max(bonuscheques.datetime)::timestamp) as recency_score,
...
--count_monetery_mode в количестве 11 особо ни о чем не говорит, а count_frequency_mode в количестве 3784 говорит, что более трети клиентов делают лишь один заказ и никогда не возвращаются. 
--Мода 133 по count_recency_mode говорит, что небольшая часть клиентов была два дня назад. Это не особо информативно. Посмотрим, сколько клиентов было 0 и 1 день назад:
...
where frequency_score = 1) / sum(monetery_score)) * 100,
	2) as "% monetery"
from
	rfm

--около 12 процентов составляет сумма, принесенная клиентами, которые сделали лишь один заказ. 
--12 процентов относительно всей суммы цифра немалая, но учитывая, что ее принесли более одной трети клиентов, а значит остальные деньги принесли другие две-трети клиентов, 
--тех, кто был лишь один раз и сделал лишь один заказ при построении итогового RFM-запроса мы отсеивать не будем - любой клиент, даже сделавший один заказ, в будущем это потенциальный постоянный клиент. 
--Так как есть разбросы в медианах и средних значениях, посмотрим и сравним данные значения с усеченными данными, уберем по 5% от минимума и максимума. Запрос:
...
group by
	card, max_date),	
ntiled_metrics AS (
select recency_score, frequency_score, monetery_score,
		NTILE(20) OVER (ORDER BY recency_score) AS ntiled_recency,
		NTILE(20) OVER (ORDER BY frequency_score) AS ntiled_frequency,
		NTILE(20) OVER (ORDER BY monetery_score) AS ntiled_monetery,
...
from
 ntiled_metrics
where (ntiled_recency BETWEEN 2 AND 19) and (ntiled_frequency BETWEEN 2 AND 19) and (ntiled_monetery BETWEEN 2 AND 19)
union
select
	round(avg(recency_score::numeric),
	2) as recency_mean,
...
--Среднее усеченное значение по monetery приближено к обычному, но смысла его как-то выделять нет - товаров с разными ценами много, заказов много, потому и расхождения. 
...
--смотреть график graf1.

--Второй распределение recency и monetery относительно frequency:

--смотреть график graf2.

И третий распределение frequency и monetery относительно recency:

--смотреть график graf3.
...
--Отдельным запросом рассмотрим количество клиентов, условно разделив на группы по дням, которые скажут нам, как давно клиент сделал свой последний заказ. 
--Так как максимальное количество дней 331 (recency_max), пусть будет четыре группы: менее 100 (ближе к среднему 112,92 по recency) дней назад, от 100 до 200, от 200 до 300, и от 300 дней. Запрос:
...
--5100 клиентов посетили аптеку за последние сто дней, это более половины из всех. Посмотрим на график с медианой (черная линия) средним (красная линия) по recency_score:

--смотреть график graf4.

те же самые среднее и медиана. Еще один график:

--смотреть график graf5.
...
--По метрикам данная группа хотя и показала вполне логичные значения по среднему и медиане (люди не часто ходят в аптеки), все же разброс довольно велик - 217 заказов как максимальное. 
--Поэтому будем исходить из среднего:
...
----смотреть график graf6.

те же среднее 4 и медиана 2, что в запросе выше по метрикам. 
Снова применим график boxplot, теперь уже по группе frequency_score:

--смотреть график graf7.
...
--смотреть график graf8, смотреть график graf9.

--По обоим графикам видно, что большинство клиентов тратят менее 20000 в аптеке, хотя и есть выброс в более 160000. 
--Продолжим условно, в тоже время исходя из графиков, деление клиентов на группы и посмотрим на их количество. Запрос:
...
select
	count(case when monetery_score < 20000 then monetery_score end) as "< 20000",
		count(case when monetery_score >= 20000 and monetery_score < 40000 then frequency_score end) as ">= 20000 and < 40000",
		count(case when monetery_score >= 40000 and monetery_score < 100000 then frequency_score end) as ">= 40000 and < 100000",
...
--Исходя из проделанного анализа составим RFM-запрос. Цифрой “1” обозначим тех клиентов, 
--кто был в аптеке не позднее 90 дней (медианное значение по recency) относительно максимальной даты по всей базе данных, сделал не менее 50 заказов, 
--а также принесшие аптеке не менее 100000. Цифрой “3” обозначим тех клиентов, 
--которые были в аптеке более 180 дней (75-й перцентиль по боксплоту) назад, кто сделал менее 5 заказов (среднее по frequency_score). 
--Минимальное качественное значение по monetery_score, исходя из анализа преобразованных данных, возьмем медиану, округлив до 1500. Цифрой “2” обозначим всех остальных. Запрос:
...
--Клиенты “262be3a0-1838-4e35-804c-ad5bf22c4fe7”, “2000200196600”, “2000200189985”, “2000200204541”, “2000200170860”, “2000200196556” с качеством “111” являются самыми важными 
--и в большей степени показательными или идеальными: 
--лишь один был 9 дней назад, лишь у двух заказов меньше 70, но все шесть принесли более 100000 каждый. 
--Такими же показательными являются клиенты с качеством “333”: были очень давно, заказов мало, денег приносят мало. Отдельным запросом можно достать  количество клиентов 
--и средние значения во всех качественных группах:
